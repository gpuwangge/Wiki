...
选择Binary distributions里面的Windows x64 ZIP  
比如：  
...
# 如果拿到一份源代码，应该如何编译？
## 第一步：构建系统
首先，要编译这份代码，需要一个构建系统 build system。  
makefile 就是一种构建系统。还有其他的构建系统。  
选择用哪种构建系统，跟目标平台（windows，linux，android or macos）有关。  
如果不想绑定在某一个目标平台上，可以使用跨平台的工具，比如 cmake。  
cmake 是构建脚本（build script），目的就是生成构建系统。它不管目标平台是什么，代码（cmakelists.txt）没有什么区别。  
如果代码（cmakelists.txt）运行在 windows 上，就生成 windows 版的构建系统，若运行在 linux 上，就生成 linux 版的构建系统。  

## 第二步: 配置系统
在写出了正确的构建脚本之后（通常需要正确设置 cmake 变量），还需要进行正确的配置。  
这里正确的配置分为两部分，一部分是编译期间的宏定义；另一部分是运行时的环境变量设置。  

软件的生命周期（配置 → 编译 → 运行）  
- CMake 变量（set()）：配置阶段（CMake 运行时），俗称 CMake 内部变量 / 缓存变量，生成构建文件后消失。  
CMake 变量除了用 set，也可以用 -Dxxx=“abc” 的方式通过命令行设置。  
- 编译宏定义（add_compile_definitions()）：编译阶段（编译器工作时），俗称预处理器宏（Macro），固化在二进制文件中。  
- 运行时环境变量（export / setx）：运行阶段（程序执行时），俗称系统环境变量，程序启动时读取，结束即消失。  

三者的代码访问方式  
- CMake变量：仅在 CMakeLists.txt 中用 ${VAR}，如果要修改要重新运行 CMake 配置。  
- 编译宏定义：在 C/C++ 代码中用 #ifdef，如果要修改必须重新编译代码。  
- 环境变量：在代码中用 getenv()，无需编译，直接修改。  

如何知道需要什么宏定义呢？  
在源代码中搜索 “#ifdef” 之类的关键词会给出提示。有一些宏定义缺失也无所谓，但有一些不行。  
如果看到一个宏定义并判断它需要被设置，就要用 add_compile_definitions() 之类的命令告诉构建脚本：你在生成构建系统的时候一定要记得设置这个宏。  

如何知道需要什么环境变量呢？  
在源代码中搜索 “getenv()” 之类的关键词，然后通过如下方式设定：  
在 linux/macos/unix-like 环境下运行这个指令   
```
export SOME_ENV=value
```
然后在当前 terminal session 里面这个环境变量就生效了   
如果 linux 里面需要永久设定某个环境变量，可以更改 ~/.bachrc 文件   

在 windows 下运行如下指令   
```
set SOME_ENV=value
```
然后它会在当前的 windows terminal 窗口下生效   
在 windows 下也可以运行如下指令  
```
setx SOME_ENV “value”
```
这个命令会写入注册表，所以它是永久生效的。你需要新打开一个命令行窗口使其生效  
另外，也可以靠 UI 配置 windows 环境变量来设定。  

在一些特殊情况下，宏定义在构建脚本中是由变量控制的，也就是说生成的宏跟 cmake 变量有关系。  
因此，这时候就要保证设置的 cmake 变量正确，生成的宏才会正确。  

只有CMake变量/编译宏定义/环境变量都设置正确的情况下，编译生成的代码才能正确运行。否则，即使构建系统成功编译了二进制文件，也有可能出现运行错误。  
