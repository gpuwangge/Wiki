...
二者的代码访问方式  
CMake 变量：仅在 CMakeLists.txt 中用 ${VAR}，如果要修改要重新运行 CMake 配置。  
编译宏定义：在 C/C++ 代码中用 #ifdef，如果要修改必须重新编译代码。  
环境变量：在代码中用 getenv()，无需编译，直接修改。  

如何知道需要什么宏定义呢？在源代码中搜索 “#ifdef” 之类的关键词会给出提示。有一些宏定义缺失也无所谓，但有一些不行。  
如果看到一个宏定义并判断它需要被设置，就要用 add_compile_definitions() 之类的命令告诉构建脚本：你在生成构建系统的时候一定要记得设置这个宏。  

如何知道需要什么环境变量呢？在源代码中搜索 “getenv()” 之类的关键词，然后通过如下方式设定：  
