...
## 全局光照
全局光照（Global Illumination，简称 GI） 是计算机图形学中用于模拟光线在真实世界中传播与多次弹射的总称。  

简单来说，它的核心公式就是：  
全局光照（GI） = 直接光照（Direct Illumination） + 间接光照（Indirect Illumination）  

核心概念对比  
- 直接光照（Direct Illumination）
光线从光源（如太阳、灯泡）发射后，直接击中物体表面并反射到人眼/相机中。  
效果：只能照亮光源能直接看到的区域，没被照到的地方会变成纯黑的阴影。  

- 间接光照（Indirect Illumination）  
光线击中第一个物体后并没有消失，而是像乒乓球一样在场景的各种物体表面继续反弹（二次弹射、多次弹射），把第一个物体的颜色和光亮带到原本照射不到的阴影区域。  

GI 包含的关键视觉效果  
- 光线二次/多次反弹（Bounce Light）：走进暗室，虽然阳光只照到窗台，但整个房间都会被泛光微微照亮。
- 颜色溢出（Color Bleeding）：将一团红色的球体靠近白墙，白墙靠近球体的一侧会被照上淡淡的红色（红球反射的红光弹到了墙上）。
- 环境光遮蔽（Ambient Occlusion, AO）：墙角、物体缝隙或接触面等光线难以弹进去的狭窄区域，会自然呈现出细致的暗部阴影。
- 焦散（Caustics）：光线穿过透明水杯或玻璃球折射后，在桌面上汇聚形成的强光斑。

常见实现方案  
- 离线/烘焙类：光照贴图（Lightmaps）、光照探针（Light Probes）、辐照度缓存（Irradiance Caching）。
- 实时近似类：SSAO/GTAO（屏幕空间环境光遮蔽）、SSGI（屏幕空间全局光照）、DDGI（动态漫反射全局光照）。
- 光线追踪类：Path Tracing（路径追踪）、ReSTIR GI、Lumen（虚幻5的混合 GI 架构）。
