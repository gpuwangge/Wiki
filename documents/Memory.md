...
问题1：为什么使用页。  
一页是内存模型内存操作的基本单位。当创建内存的时候，最少也会创建一页。之后如果需要的地址都在一页内，就不用再创建了。  
这样的好处是，仅创建需要的内存地址。（可用的内存地址不会变少）  
当然页的位数也要设置合适的值。如果太小，就需要频繁创建页面；如果太大，则容易造成浪费，并且page offset的长度也会变长(需要更多的位数存储address_lo)  

问题2：对于12位page offset, 52位page index，c语言本身支持64位数据，为什么要把page index继续分层呢？  
如果不分层，则准备52位的address_hi作为index。内存树会在一开始创建长度为2^52的Node Table。(根据需要，下面会在其中一个Node下Create Page)
如果分成30-22两层，则创建长度为2^30的第一层Node Table，再(在其中一个Node下)创建长度为2^22的第二层Node Table。
2^52=4.5035996e+15  
2^30=1073741824  
2^22=4194304  
2^52-2^30-2^22=4.5035985e+15  
可以看出，如果采取不分层的策略，则内存模型需要在一开始就创建数量巨大的Node Pointer。
(假设一个Node包含Node指针, Page指针和2个Function指针，那在64位系统里就是2^(6+2)=256 bits的size。如果创建这个Node表，会产生多大的数据呢？)
(考虑到2^52=4e15,总size就是2^60 bits。1t的硬盘也就2^40 bits。所以2^60的数据)
(这还是仅考虑空的Node Table,还没算Page的大小呢)
如果分成30-20两层，创建的数量就为2^38+2^30<3e11，不超过1t。  

问题3：为什么一开始就要创建每一层的所有的Node Table?  
